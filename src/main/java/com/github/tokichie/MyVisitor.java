  ...
  public void preVisit(ASTNode node) {
    ...
    for (StructuralPropertyDescriptor descriptor : structualProperties) {
      if (descriptor.isSimpleProperty()) {
        ...
        * else if (descriptor.isChildProperty()) { ASTNode childNode =
        * (ASTNode)node.getStructuralProperty(descriptor); if (childNode != null) {
        * childNode.accept(this); } } else if (descriptor.isChildListProperty()) { List<ASTNode>
        * childNodes = (List<ASTNode>)node.getStructuralProperty(descriptor); if (childNodes !=
        * null) { for (ASTNode childNode : childNodes) { childNode.accept(this); } } }
        */
    }
